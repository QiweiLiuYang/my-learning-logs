...
### Debugging
Una de las formas de facilitar el desarrollo es meter logs con:
- console.log() // Imprime un mensaje en la consola
- console.warn() // Imprime un aviso en la consola
- console.error() // Imprime un error en la consola

Estos logs no deberían de aparecer una vez en producción.

### Browser Debugger
La mayoría de navegadores modernos traen un debugger que podemos ver el código que se está corriendo en la web que visitamos.
Tenemos la keyword **debugger** para poner un **breakpoint** y parar el código en ese punto.